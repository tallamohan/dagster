...
class AssetsDefinition(ResourceAddable, RequiresResources, IHasInternalInit):
    ...
    def has_output_for_asset_key(self, key: AssetKey) -> bool:
        return key in self.output_name_by_asset_key

    def get_output_name_for_asset_key(self, key: AssetKey) -> str:
        try:
            return self.output_name_by_asset_key[key]
        except KeyError:
    ...
    @cached_property
    def output_name_by_asset_key(self) -> Mapping[AssetKey, str]:
        return {key: output_name for output_name, key in self.keys_by_output_name.items()}
