...
class AssetCheckExecutionRecord(
    ...
    def from_db_row(cls, row) -> "AssetCheckExecutionRecord":
        return cls(
            key=AssetCheckKey(row["key"], row["check_name"]),
            id=row["id"],
            run_id=row["run_id"],
    ...
    def resolve_status(self, loading_context: LoadingContext) -> AssetCheckExecutionResolvedStatus:
        if self.status == AssetCheckExecutionRecordStatus.SUCCEEDED:
            ...
            run_record = RunRecord.get(loading_context, self.run_id)
            if not run_record:
                # Run deleted
    ...
    def targets_latest_materialization(self, loading_context: LoadingContext) -> bool:
        ...
        asset_record = AssetRecord.get(loading_context, self.key.asset_key)
        ...
        if resolved_status in [
            ...
            latest_materialization_run_record = RunRecord.get(
                loading_context, latest_materialization_run_id
            )
            check_run_record = RunRecord.get(loading_context, self.run_id)
            return bool(
