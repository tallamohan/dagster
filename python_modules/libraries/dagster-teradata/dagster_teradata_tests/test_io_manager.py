...
from dagster_teradata.teradata_io_manager import TeradataDbClient, _get_cleanup_statement


def test_get_select_statement():
    assert (
        TeradataDbClient.get_select_statement(
            TableSlice(database="database_abc", table="table1")
        )
        == "SELECT * FROM database_abc.table1"
    )
...
def test_get_select_statement_columns():
    assert (
        ...
        == "SELECT apple, banana FROM database_abc.table1"
    )
...
def test_get_select_statement_time_partitioned():
    assert (
        ...
        == "SELECT apple, banana FROM database_abc.table1 WHERE\nmy_timestamp_col >="
...
def test_get_select_statement_multiple_static_partitions():
    assert (
        ...
        == "SELECT fruit_col, other_col FROM database_abc.table1 WHERE\nfruit_col in"
        " ('apple', 'banana')"
    )
...
def test_get_select_statement_multi_partitioned():
    assert (
        ...
        == "SELECT * FROM database_abc.table1 WHERE\nmy_fruit_col in ('apple')"
...
def test_get_cleanup_statement():
    assert (
        ...
        == "DELETE FROM database_abc.table1"
    )
...
def test_get_cleanup_statement_time_partitioned():
    assert (
        ...
        == "DELETE FROM database_abc.table1 WHERE\nmy_timestamp_col >= '2020-01-02"
